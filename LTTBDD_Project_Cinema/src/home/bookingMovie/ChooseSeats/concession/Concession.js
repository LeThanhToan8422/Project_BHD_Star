...
const Concession = ({ navigation, route }) => {
  ...
  useEffect(() => {
    setPricesPayment(concession.reduce((total, c2, index) => {
      return total + c2.prices*quantityConcession[index]
    }, 0))
  }, [quantityConcession])

  useEffect(() => {
    setConcessionChosen(concession.map((cs, index) => {
  ...
  let handlePressIconSub = (index) => {
    setQuantityConcession(prevQuantity => {
        ...
        newQuantity[index] === 0 ? newQuantity[index] = 0 : newQuantity[index] -= 1
        return newQuantity
    })
  }
    ...
    >
      ...
      >
        ...
        {concession.map((cs, index) => (
          <View style={styles.viewConcession}>
            <Image source={cs.src} style={{height : 80, width : 80, marginRight : 20}}/>
            <View style={{ flexDirection: "column" }}>
              <Text style={{color : 'white', fontSize : 18, width : 250}} numberOfLines={2}>{cs.name}</Text>
              <View style={{ width : 240, flexDirection: "row", justifyContent : 'space-between' }}>
                <View style={{ flexDirection: "row" }}>
                    <Ionicons name="remove-circle-outline" size={22} color={'#c2c2c2'}
                    onPress={() => handlePressIconSub(index)}
                    />
                    <Text style={{color : 'white', marginHorizontal : 10}}>{quantityConcession[index]}</Text>
                    <Ionicons name="add-circle-outline" size={22} color={'#63b133'}
                    onPress={() => handlePressIconPlus(index)}
                    />
                </View>
                <Text style={{color : 'white'}}>{cs.prices.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</Text>
              </View>
            </View>
          </View>
        ))}
